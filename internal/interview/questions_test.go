...
func TestIRCRefsPopulated(t *testing.T) {
	tests := []struct {
		...
		{"w2:1:wages", "IRC \u00a761(a)"},
		{"w2:1:federal_tax_withheld", "IRC \u00a73402"},
		{"w2:1:ss_wages", "IRC \u00a73121"},
		{"w2:1:medicare_wages", "IRC \u00a73101"},
		{"schedule_a:1", "IRC \u00a7213(a)"},
		{"schedule_a:5a", "IRC \u00a7164"},
		{"schedule_a:8a", "IRC \u00a7163(h)"},
		{"schedule_a:12", "IRC \u00a7170"},
		{"1099int:1:interest_income", "IRC \u00a761(a)(4)"},
		{"1099int:1:us_savings_bond_interest", "IRC \u00a7103"},
		{"1099int:1:tax_exempt_interest", "IRC \u00a7103"},
		{"1099div:1:ordinary_dividends", "IRC \u00a761(a)(7)"},
		{"1099div:1:qualified_dividends", "IRC \u00a71(h)(11)"},
		{"1099div:1:section_199a_dividends", "IRC \u00a7199A"},
		{"form_8889:2", "IRC \u00a7223"},
		{"1099b:1:proceeds", "IRC \u00a71001"},
		{"1099b:1:cost_basis", "IRC \u00a71001"},
...
func TestCARefOnlyWithCA(t *testing.T) {
	...
	fieldsWithCARef := []struct {
		...
		{"w2:1:wages", "R&TC \u00a717071"},
		{"schedule_a:1", "R&TC \u00a717201"},
		{"schedule_a:5a", "R&TC \u00a717220 (not allowed)"},
		{"form_8889:2", "R&TC \u00a717215 (not allowed)"},
		{"1099div:1:qualified_dividends", "R&TC \u00a717041 (taxed as ordinary)"},
		{"1099b:1:proceeds", "R&TC \u00a718031"},
	}
...
func TestIRCRefAlwaysIncluded(t *testing.T) {
	...
	cp := GetContextualPrompt("w2:1:wages", "fallback", "")
	if cp.IRCRef == "" {
		t.Error("IRCRef should be populated even without state code")
	}

	cp = GetContextualPrompt("w2:1:wages", "fallback", "CA")
	if cp.IRCRef == "" {
		t.Error("IRCRef should be populated with CA state code")
	}

	cp = GetContextualPrompt("w2:1:wages", "fallback", "NY")
	if cp.IRCRef == "" {
